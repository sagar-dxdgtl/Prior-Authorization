...
| EternalHealth | AZ | Medicare Advantage | — | — | — | pdf-directory | No FHIR/NPIs — network published only as a date-stamped monthly PDF (AaNeel). URL is **discovered dynamically** from the find-a-provider page (changes each update), parsed (fmt=aaneel) into `payer_directory_entries`, matched by name+state+zip (DbDirectoryAdapter). review: Stedi id — research RP037, unconfirmed. |
