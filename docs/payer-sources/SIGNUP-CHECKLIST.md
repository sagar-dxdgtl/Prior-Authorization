...
| **EternalHealth** | `ETERNAL` | **No API** — date-stamped monthly PDF (AaNeel) | n/a | **none** (public PDF; URL discovered from the find-a-provider page) | Parsed (fmt=aaneel) into `payer_directory_entries`, matched by name+state+zip — PDF has no NPIs. App-scheduled monthly refresh (`ENABLE_DIRECTORY_REFRESH=1`); manual: `python -m network_probe.cli.load_directory eternalhealth-az`. | — |
