...
_DIRECTORY_CAVEAT = (
    "Single payer-directory source; directories are frequently stale "
...
class NppesSource:
    ...
    def check(self, q: ProviderQuery, verdict: NetworkVerdict) -> Signal | None:
        ...
        if not basic:  # npiDetails returns {} for an unknown / deactivated NPI
            return Signal(self.name, "contradicts", f"NPI {q.npi} is not active in NPPES (not found / deactivated).")
        ...
        who = (
            " ".join(x for x in [basic.get("firstName", ""), basic.get("lastName", "")] if x).strip()
            or basic.get("organizationName", "")
            or "provider"
        )
        ...
        if q.state and states and q.state.upper() not in states:
            return Signal(
                self.name, "inconclusive", f"NPPES practice states {sorted(states)} don't include {q.state.upper()}."
            )
    ...
    def check(self, q: ProviderQuery, verdict: NetworkVerdict) -> Signal | None:
        ...
        if vs:
            ...
            if vs.status == NetworkStatus.OUT_OF_NETWORK.value:
                ...
                tail = (
                    "Confirms the directory's out-of-network finding."
                    if agrees
                    else "Directory listed the provider, but the billing TIN is out-of-network — review."
                )
                return Signal(
                    self.name,
                    "corroborates" if agrees else "contradicts",
...
class StediSource:
    ...
    *benefit-tier*, and only some payers return a provider-specific network status — so this is a
        ...
        body = {
            ...
            "subscriber": {
                ...
                for k, v in {
                    ...
                    "dateOfBirth": self._to_stedi_dob(q.dob),
    ...
    def _interpret(data: dict) -> Signal:
        codes = {
            b.get("inPlanNetworkIndicatorCode")
            for b in (data.get("benefitsInformation") or [])
        ...
        return Signal("Stedi", "inconclusive", "271 carried no provider-specific in-network signal (payer-dependent).")
...
class StediMockSource:
    ...
    def check(self, q: ProviderQuery, verdict: NetworkVerdict) -> Signal | None:
        ...
        if data is None:
            return Signal(
                self.name, "inconclusive", "271 carried no provider-specific in-network signal (payer-dependent)."
            )
    ...
    if verdict.status == NetworkStatus.IN_NETWORK:
        if contradictions:
            ...
            verdict.notes = (
                ...
                + " Flagged for human verification. (Original: "
                + verdict.notes
                + ")"
            )
        else:
