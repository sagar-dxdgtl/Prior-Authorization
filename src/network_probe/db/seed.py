...
def ensure_demo_tenant_admin(conn, initial_password: str = "ChangeMe-Admin-2026") -> None:
    ...
    if tid is None:
        tid = uuid.uuid4()
        conn.execute(
            text("INSERT INTO tenants (id, name, slug, created_at) VALUES (:id,'Demo Practice',:s, now())"),
            {"id": str(tid), "s": DEMO_SLUG},
        )
    exists = conn.execute(
        text("SELECT 1 FROM users WHERE lower(username)=lower(:u)"), {"u": DEMO_ADMIN_USERNAME}
    ).scalar()
    if not exists:
        conn.execute(
            text(
            ...
            {"id": str(uuid.uuid4()), "tid": str(tid), "u": DEMO_ADMIN_USERNAME, "ph": hash_password(initial_password)},
        )
