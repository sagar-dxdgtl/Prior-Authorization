...
def test_tenant_cannot_read_other_tenants_rows():
    ...
    with Session(owner_engine()) as s:
        s.add_all([Tenant(id=a, name="A", slug=f"a-{a.hex[:6]}"), Tenant(id=b, name="B", slug=f"b-{b.hex[:6]}")])
        s.commit()
    # write a row under tenant A
    with tenant_session(a) as s:
        s.add(
            OverrideRow(
                tenant_id=a, payer="oscar", npi="1", status="IN_NETWORK", verified_by="t", verified_at="2026-01-01"
            )
        )
...
def test_app_role_cannot_bypass_with_no_tenant_context():
    ...
    with Session(owner_engine()) as s:
        s.add_all([Tenant(id=a, name="A", slug=f"a-{a.hex[:6]}"), Tenant(id=c, name="C", slug=f"c-{c.hex[:6]}")])
        s.commit()
    with tenant_session(a) as s:
        s.add(
            OverrideRow(
                tenant_id=a, payer="cigna", npi="2", status="OUT_OF_NETWORK", verified_by="t", verified_at="2026-02-01"
            )
        )
