...
def test_blocks_hostname_resolving_internal(monkeypatch):
    monkeypatch.setattr(
        "network_probe.api.netutil.socket.getaddrinfo", lambda host, *a, **k: [(2, 1, 6, "", ("127.0.0.1", 0))]
    )
    with pytest.raises(ValueError):
...
def test_allows_public_hostname(monkeypatch):
    monkeypatch.setattr(
        "network_probe.api.netutil.socket.getaddrinfo", lambda host, *a, **k: [(2, 1, 6, "", ("93.184.216.34", 0))]
    )
