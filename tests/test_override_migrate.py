...
def test_db_override_store_add_lookup_and_isolation(demo_tenant):
    ...
    DbOverrideStore(demo_tenant).add(
        Override(
            payer="devoted", npi="1629339312", status="OUT_OF_NETWORK", verified_by="Availity", verified_at="2026-05-21"
        )
    )
    ...
    with Session(owner_engine()) as s:
        s.add(Tenant(id=other, name="Other", slug=f"other-{other.hex[:6]}"))
        s.commit()
...
def test_migrate_json_to_db_idempotent(tmp_path, demo_tenant):
    ...
    assert (
        DbOverrideStore(demo_tenant).lookup(ProviderQuery(payer="devoted", plan_hint="", npi="1629339312")).status
        == "OUT_OF_NETWORK"
    )
...
def test_check_eligibility_applies_tenant_override(demo_tenant):
    ...
    try:
        r = e.check_eligibility(
            ProviderQuery(payer="devoted", plan_hint="PPO", npi="1629339312"),
            catalogue=FakeCat(),
            stedi=FakeStedi(),
