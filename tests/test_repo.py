...
def test_add_rejects_tenant_id_in_kwargs(demo_tenant):
    ...
    with tenant_session(demo_tenant) as s:
        ...
        with pytest.raises(TypeError):
            repo.add(tenant_id=uuid.uuid4(), payer="x", npi="1", status="IN_NETWORK",
                     verified_by="a", verified_at="2026-01-01")

@pytest.mark.db
def test_eligibility_check_repo_records(demo_tenant):
    with tenant_session(demo_tenant) as s:
        repo = EligibilityCheckRepo(s, demo_tenant)
        row = repo.record(action="eligibility", payer_key="oscar", status="IN_NETWORK",
                          result_jsonb={"ok": True}, source_audit={})
