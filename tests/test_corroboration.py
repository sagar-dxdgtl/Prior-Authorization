...
def _verdict(status, conf="high", name="Jing Li, MD", npi="1629339312"):
    return NetworkVerdict(
        status=status,
        matched_provider={"npi": npi, "name": name},
...
def test_single_source_in_demoted_to_medium():
    v = finalize(
        _verdict(NetworkStatus.IN_NETWORK, "high"),
        _q(),
        sources=[_FakeSource(Signal("FAKE", "corroborates", "ok"))],
        override_store=_NO,
    )
...
def test_oon_is_left_strong():
    v = finalize(
        _verdict(NetworkStatus.OUT_OF_NETWORK, "high"),
        _q(),
        sources=[_FakeSource(Signal("FAKE", "corroborates", "ok"))],
        override_store=_NO,
    )
...
def test_contradiction_flips_in_to_review():
    v = finalize(
        _verdict(NetworkStatus.IN_NETWORK, "high"),
        _q(),
        sources=[_FakeSource(Signal("NPPES", "contradicts", "NPI deactivated"))],
        override_store=_NO,
    )
...
def test_unreachable_source_does_not_flip():
    v = finalize(
        _verdict(NetworkStatus.IN_NETWORK, "high"),
        _q(),
        sources=[_FakeSource(Signal("NPPES", "inconclusive", "unreachable"))],
        override_store=_NO,
    )
    assert v.status == NetworkStatus.IN_NETWORK
...
def test_nppes_active_corroborates():
    body = {
        "basic": {"firstName": "Jing", "lastName": "Li", "status": "A"},
        "addresses": [{"state": "CO"}],
...
def test_nppes_deactivated_contradicts():
    body = {"basic": {"firstName": "Jing", "lastName": "Li", "status": "D"}, "addresses": [{"state": "CO"}]}
    s = _nppes(body).check(_q(), _verdict(NetworkStatus.IN_NETWORK))
...
def test_nppes_identity_mismatch_contradicts():
    body = {"basic": {"firstName": "Robert", "lastName": "Smith", "status": "A"}, "addresses": [{"state": "CO"}]}
...
def test_verified_tin_status_contradicts_when_directory_says_in():
    ...
    assert s.result == "contradicts"
    out = finalize(
        _verdict(NetworkStatus.IN_NETWORK, name="William Kiang", npi="1184610453"),
        q,
        sources=[TinScopeSource()],
        override_store=_NO,
    )
...
def test_override_wins_over_directory(tmp_path):
    ...
    store.add(
        Override(
            ...
            verified_by="Availity 2026-05-21",
            verified_at="2026-05-21",
            plan="PPO",
            note="confirmed OON by payer rep",
        )
    )
...
def test_override_lookup_specificity(tmp_path):
    store = OverrideStore(path=tmp_path / "ov.json")
    store.add(
        Override(payer="devoted", npi="1629339312", status="OUT_OF_NETWORK", verified_by="x", verified_at="2026-05-21")
    )
...
def test_stedi_interpret_network_codes():
    ...
    assert (
        f({"benefitsInformation": [{"inPlanNetworkIndicatorCode": "Y"}, {"inPlanNetworkIndicatorCode": "N"}]}).result
        == "inconclusive"
    )
...
def test_run_display_signals_collects_from_each_source():
    sigs = run_display_signals(
        _verdict(NetworkStatus.IN_NETWORK), _q(), [_FakeSource(Signal("FAKE", "corroborates", "ok")), StediMockSource()]
    )
...
def test_finalize_accepts_precomputed_signals_without_rerunning():
    ...
    v = finalize(_verdict(NetworkStatus.IN_NETWORK, "high"), _q(), sources=[_Boom()], override_store=_NO, signals=pre)
