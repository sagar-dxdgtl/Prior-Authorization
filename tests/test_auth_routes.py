...
def test_first_login_then_change_password(seed_admin):
    ...
    body = c.post("/api/auth/login", data={"grant_type":"password","username":"admin","password":"Brand-new-pw-456"}).json()
...
def test_bad_password_generic_no_user_leak(seed_admin):
    ...
    assert c.post("/api/auth/login", data={"grant_type":"password","username":"ghost","password":"nope"}).status_code == 401

@pytest.mark.db
def test_lockout_after_threshold(seed_admin):
    c = TestClient(_app())
    for _ in range(5):
        assert c.post("/api/auth/login", data={"grant_type":"password","username":"admin","password":"wrong"}).status_code == 401
    r = c.post("/api/auth/login", data={"grant_type":"password","username":"admin","password":"Initial-pw-1234"})
...
def test_refresh_issues_new_access(seed_admin):
    c = TestClient(_app())
    acc = c.post("/api/auth/login", data={"grant_type":"password","username":"admin","password":"Initial-pw-1234"}).json()["tokens"]["access"]
    c.post("/api/auth/change-password/", headers={"Authorization":f"Bearer {acc}"},
           json={"current_password":"Initial-pw-1234","new_password":"Brand-new-pw-456","confirm_password":"Brand-new-pw-456"})
    body = c.post("/api/auth/login", data={"grant_type":"password","username":"admin","password":"Brand-new-pw-456"}).json()
...
def test_pre_change_token_rejected_after_change(seed_admin):
    ...
    r = c.post("/api/auth/change-password/", headers={"Authorization":f"Bearer {acc}"},
